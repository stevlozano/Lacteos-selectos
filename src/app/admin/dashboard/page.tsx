        ...
        <div className="flex items-center justify-around">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => setCurrentView(item.id)}
              className={`relative flex items-center justify-center py-3 flex-1 transition-colors
                ${currentView === item.id
                  ? 'text-black dark:text-white'
                  : 'text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300'
                }`}
            >
              {item.icon === 'space_dashboard' && <DashboardIcon size={22} />}
              {item.icon === 'inventory_2' && <InventoryIcon size={22} />}
              {item.icon === 'receipt_long' && <ReceiptIcon size={22} />}
              {item.icon === 'person' && <PersonIcon size={22} />}
              {item.icon === 'settings' && <span className="material-symbols-outlined text-[22px]">settings</span>}
              {currentView === item.id && (
                <span className="absolute bottom-1 w-8 h-0.5 bg-black dark:bg-white rounded-t-full" />
              )}
            </button>
          ))}
        </div>
